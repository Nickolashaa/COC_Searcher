...
class ScreenReader:
    ...
    def read(self, name):
        result = self.ocr.ocr(name, cls=False)
        if result[-1] is None:
            return 0
        num = result[-1][-1][-1][0]
    ...
    def screen(self, region: tuple, coords: list, names: list):
        os.system("cd platform-tools && adb -s 127.0.0.1:5555 shell screencap -p /sdcard/screen.png")
        os.system("cd platform-tools && adb -s 127.0.0.1:5555 pull /sdcard/screen.png")
        img = Image.open('platform-tools/screen.png')
        img = img.crop(region)
        for i in range(3):
            crop_box = (
                coords[i][0],
    ...
    def next(self):
        os.system("cd platform-tools && adb -s 127.0.0.1:5555 shell input tap 1500 675")
        time.sleep(1.5)
...
def parsing(gold, elixir, dark_elixir, main_self):
    ...
    while not main_self.stop_event.is_set():
        main_self.reader.screen(
            ...
            names=['app/parse_pic/gold.png', 'app/parse_pic/elixir.png', 'app/parse_pic/dark_elixir.png']
        )
        print(1)
